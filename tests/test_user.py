...
def test_user(db_connection):
    ...
    try:
        ...
        pytest.fail(f"Error al crear usuario de prueba: {e}")
    
    yield user_id  # Devuelve el ID del usuario de prueba
...
def test_asignar_socio_ya_existente(test_user):
    ...
    # Primero asignamos al usuario como socio
...
def test_socio_se_guarda_correctamente(test_user):
    ...
    # Asignamos el usuario como socio
    UserModel.asignar_socio(test_user)

    # Verificamos que el usuario realmente esté en la tabla `socios`
...
def test_limpieza_de_pruebas(test_user):
    ...
    # Primero asignamos el usuario como socio
    UserModel.asignar_socio(test_user)
